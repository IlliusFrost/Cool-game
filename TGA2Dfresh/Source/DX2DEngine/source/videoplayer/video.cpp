...
bool CVideo::Init(const char* aPath, bool aPlayAudio)
{
	...
	if (!myShaderResource)
	{
		...
		Tga2D::Vector2f texSize = Tga2D::Vector2f(static_cast<float>(mySize.x), static_cast<float>(mySize.y));
		...
		mySprite->SetSizeRelativeToScreen(Tga2D::Vector2f(texSize.x, texSize.y));
