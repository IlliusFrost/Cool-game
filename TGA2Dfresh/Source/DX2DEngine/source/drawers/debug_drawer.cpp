...
void CDebugDrawer::Init()
{
	...
	myFPS->SetText("--");
	myFPS->SetPosition(Tga2D::Vector2f(0, 0.035f));
	myFPS->SetColor({ 1, 1, 1, 1.0f });
	myFPS->SetScale(theSize);

	myMemUsage = new Tga2D::CText("Text/arial.ttf");
	myMemUsage->SetText("--");
	myMemUsage->SetPosition(Tga2D::Vector2f(0.0f, 0.06f));
	...
	myErrorsText->SetPosition(Tga2D::Vector2f(0.0f, 0.15f));
...
void CDebugDrawer::DrawCircle(Vector2f aPos, float aRadius, CColor aColor)
{
	...
	for (int i = 0; i < circleResolution+1; i++)
	{
		...
		{
			first = true;
			to = Tga2D::Vector2f( aRadius*cos(angle) + aPos.x, (aRadius*windowRatio)*sin(angle) + aPos.y );
			continue;
		}
		myLineBuffer[myNumberOfRenderedLines].myFromPosition = to;
		myLineBuffer[myNumberOfRenderedLines].myToPosition = Tga2D::Vector2f(aRadius*cos(angle) + aPos.x, (aRadius*windowRatio)*sin(angle) + aPos.y);
		myLineBuffer[myNumberOfRenderedLines].myColor = aColor.AsVec4();
		to = myLineBuffer[myNumberOfRenderedLines].myToPosition;
		myNumberOfRenderedLines++;
