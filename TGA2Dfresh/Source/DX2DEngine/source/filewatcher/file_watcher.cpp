...
{
	if (std::experimental::filesystem::exists(aFilePath) == false)
	{
		return 0;
	}
	return std::experimental::filesystem::last_write_time(aFilePath).time_since_epoch().count();
}
...
{
	return std::experimental::filesystem::exists(aFilePath);
}
