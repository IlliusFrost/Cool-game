...
bool AudioOut::Play(Handle hndl, Handle& outChannel)
{
	if (!hndl)
		return false;
...
void AudioOut::SetPosition(const Handle& hndl, Tga2D::Vector2f aPosition)
{
...
AudioOut::Handle AudioOut::Load(const std::string& fname, bool repeat)
{
  ...
  if (!hndl)
    return audio_helpers::PrintBassError("BASS_SampleLoad", fname);
  else
  {
